                  ...
                  shinyBS::bsCollapsePanel("Select variables",
                    ...
                    selectizeInput("scenarioFby",  label=h4("Scenarios"), unique(fltStk$scenario),  selected=unique(fltStk$scenario)[1], multiple=T, options=list(plugins=list("remove_button", "drag_drop")))
                   			  ...
                   			  shinyBS::bsCollapsePanel("Graphical options",
    			                           ...
    			                           fluidRow(column(3,checkboxInput("fitFby", "Free scales", FALSE)),
    			                                    column(3,numericInput('nColFby', h5("Number of columns in facets"), value = 2, min = 1, max = 10, step = 1, width = 200))),
    			                           fluidRow(column(3,checkboxInput("dotLineFby", "Dot & Lines", FALSE)),
    			                                    column(3,numericInput('lwdFby', h5("Line width"), value = 1, min = 0, max = 5, step = 0.1, width = 100)),
    			                                    column(3,numericInput('dszFby', h5("Dot size"), value = 3, min = 0, max = 5, step = 0.1, width = 100)))
    			  ), 
     ...
     tabPanel(
       ...
       fluidRow(
         column(3,
                ...
                fluidRow(column(3, checkboxInput("fitbyA", "Free scales", FALSE)),
                         column(3, numericInput('nColbyA', h5("Number of columns in facets"), value = 2, min = 1, max = 10, step = 1, width = 200)))
                #hr(),
                ),
                shinyBS::bsCollapsePanel("Download",
                    #  Options for file downloading
