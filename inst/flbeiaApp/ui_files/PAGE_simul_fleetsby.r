            ...
            tabPanel(
              ...
              fluidRow(
                ...
                column(3,
                       ...
                       selectizeInput("fleetbyA", label=h4("Fleet"), levels(as.factor(fltStk$fleet)), selected=levels(as.factor(fltStk$fleet))[1],multiple=F, options=list(plugins=list("remove_button", "drag_drop"))),
                       selectizeInput("stockbyA", label=h4("Stock"), levels(as.factor(fltStk$stock)), selected=levels(as.factor(fltStk$stock)),multiple=T, options=list(plugins=list("remove_button", "drag_drop"))),
                       selectizeInput("indicatorbyA", label=h4("Indicators"), c('catch', 'landings', 'discards', 'quota'),selected='catch',multiple=T, options=list(plugins=list("remove_button", "drag_drop"))),
                       selectizeInput("scenariobyA", label=h4("Scenarios"), levels(as.factor(fltStk$scenario)), selected=unique(fltStk$scenario)[1], multiple=T, options=list(plugins=list("remove_button", "drag_drop"))),
