     ...
     tabPanel(
       ...
       fluidRow(
         ...
         column(3,
                ...
                selectizeInput("indicatorbyA", label=h4("Indicators"), unique(fltStk$scenario)[which(unique(fltStk$scenario)) %in% c('catch', 'landings', 'discards', 'quota')],
                                                                       selected=unique(fltStk$scenario)[which(unique(fltStk$scenario)) %in% c('catch', 'landings', 'discards', 'quota')[1]],multiple=T, options=list(plugins=list("remove_button", "drag_drop"))),
